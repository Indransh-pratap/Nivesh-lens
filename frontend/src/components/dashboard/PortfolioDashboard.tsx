...
export function PortfolioDashboard() {
  ...
  return <div className="mx-auto max-w-2xl space-y-5 py-6">
    {error && <p role="alert" className="rounded-md border border-[var(--negative)]/30 bg-[var(--negative)]/10 p-3 text-sm text-[var(--negative)]">{error}</p>}
    {success && <p role="status" className="rounded-md border border-[var(--positive)]/30 bg-[var(--positive)]/10 p-3 text-sm text-[var(--positive)]">{success}</p>}
    {!portfolio ? <Card><CardContent className="p-6"><h1 className="text-2xl font-semibold">Welcome</h1><p className="mt-2 text-sm text-muted-foreground">You don&apos;t have a portfolio yet.</p><form className="mt-5 flex max-w-md gap-2" onSubmit={submit}><input aria-label="Portfolio name" value={name} onChange={(event) => setName(event.target.value)} maxLength={120} placeholder="My Portfolio" className="h-10 flex-1 rounded-md border border-border bg-background px-3 text-sm" disabled={saving} /><Button type="submit" disabled={saving}>{saving ? "Creating…" : "Create Portfolio"}</Button></form></CardContent></Card> : <Card><CardContent className="p-6"><h1 className="text-2xl font-semibold">{portfolio.name}</h1><div className="mt-6 grid gap-4 sm:grid-cols-3"><div><p className="text-xs uppercase text-muted-foreground">Total Value</p><p className="mt-1 text-2xl font-semibold">₹0</p></div><div><p className="text-xs uppercase text-muted-foreground">Holdings</p><p className="mt-1 text-2xl font-semibold">0</p></div><div><p className="text-xs uppercase text-muted-foreground">Transactions</p><p className="mt-1 text-2xl font-semibold">0</p></div></div><Button className="mt-6" disabled>Import CAS <span className="ml-1 text-xs">Coming soon</span></Button></CardContent></Card>}
  </div>;
}
