...
export default function HoldingsPage() {
  ...
  const [sortBy, setSortBy] = useState<"value" | "returns" | "name" | "weight">("value");
  ...
  const filteredHoldings = holdings
    .filter(h => {
      ...
      else if (activeFilter === "Mutual Fund") matchFilter = h.type === "Mutual Fund";
      else if (activeFilter === "Direct") matchFilter = h.planType === "Direct";
      else if (activeFilter === "Regular") matchFilter = h.planType === "Regular";
  ...
  const get52WRange = (currentPrice: number) => {
    ...
    const progress = Math.min(100, Math.max(5, ((currentPrice - low) / (high - low)) * 100));
    ...
    <div className="space-y-8 pb-16 text-foreground">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="p-4 rounded-2xl bg-[var(--card)] border border-border">
          <span className="text-xs text-muted-foreground">Total Holdings Value</span>
          ...
          <span className="text-[11px] text-[var(--positive)] font-finance flex items-center gap-1 mt-0.5">
            <TrendingUp className="w-3 h-3" /> +17.72% (+₹5.24L)
          </span>
        </div>

        <div className="p-4 rounded-2xl bg-[var(--card)] border border-border">
          <span className="text-xs text-muted-foreground">Total Instruments</span>
          <p className="text-xl sm:text-2xl font-bold font-finance text-foreground mt-1 tabular-nums">
        ...
        <div className="p-4 rounded-2xl bg-[var(--card)] border border-border">
          <span className="text-xs text-muted-foreground">Regular Plans Flagged</span>
        ...
        <div className="p-4 rounded-2xl bg-[var(--card)] border border-border">
          <span className="text-xs text-muted-foreground">Nominee Compliance</span>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          actionLabel={searchQuery || activeFilter !== "All" ? "Clear filters" : undefined}
          onAction={
            searchQuery || activeFilter !== "All"
              ? () => { setSearchQuery(""); setActiveFilter("All"); }
              : undefined
          }
        />
      ) : viewMode === "grid" ? (
              ...
              >
                <div>
                  ...
                  <div className="flex items-start justify-between gap-2 pb-2">
                    ...
                    <span className="px-2 py-0.5 rounded-md text-[10px] font-medium border border-border bg-accent/60 text-muted-foreground shrink-0">
                      {h.type === "Stock" ? "Equity" : `${h.planType || ""} MF`}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-3 border-t border-border font-finance text-xs">
                    <div>
                      <span className="text-[10px] uppercase font-sans text-muted-foreground block">Current Value</span>
                      <span className="text-base font-bold text-foreground tabular-nums">
                        ₹{h.currentValue.toLocaleString("en-IN")}
                      </span>
                      <span className="text-[10px] text-muted-foreground block font-sans">
                        Qty: {h.quantity} @ ₹{h.avgPrice.toFixed(1)}
                      </span>
                    </div>

                    <div className="text-right">
                      <span className="text-[10px] uppercase font-sans text-muted-foreground block">Total P&L</span>
                      <span className={cn(
                        "text-base font-bold tabular-nums",
                        isProfitable ? "text-[var(--positive)]" : "text-[var(--negative)]"
                      )}>
                        {isProfitable ? "+" : ""}{h.returns.toFixed(2)}%
                      </span>
                      <span className="text-[10px] text-muted-foreground block tabular-nums">
                        ({isProfitable ? "+" : ""}₹{h.returnsValue.toLocaleString("en-IN")})
                      </span>
                    </div>
                  </div>

                  {/* 52-Week Price Range Visualizer */}
                  <div className="pt-3 space-y-1 text-[10.5px] font-mono">
                    <div className="flex justify-between text-muted-foreground">
                      <span>52W L: ₹{range.low.toFixed(1)}</span>
                      <span className="text-foreground font-semibold">LTP: ₹{h.currentPrice.toFixed(1)}</span>
                ...
                <div className="pt-3 border-t border-border flex items-center justify-between text-xs">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-semibold text-foreground font-finance tabular-nums">{h.allocation.toFixed(1)}%</span>
                  </div>
            ...
            <table className="w-full text-left text-xs font-finance">
              <thead className="bg-accent/40 text-[10px] uppercase font-bold text-muted-foreground border-b border-border font-sans">
                <tr>
                  ...
                  <th className="py-3 px-3 text-right cursor-pointer hover:text-foreground font-bold" onClick={() => setSortBy("value")}>
                    Current Value
                  </th>
                  <th className="py-3 px-3 text-right cursor-pointer hover:text-foreground font-bold" onClick={() => setSortBy("returns")}>
                    Returns (P&L)
                  </th>
                  <th className="py-3 px-3 text-right">Weight</th>
                  <th className="py-3 px-4 font-sans text-center">Nominee</th>
                </tr>
              </thead>
                  ...
                  return (
                    ...
                    >
                      <td className="py-3 px-4 font-sans font-semibold text-foreground">
                        <div>{h.name}</div>
                        <span className="text-[10px] text-muted-foreground font-mono">{h.ticker} {h.isin ? `· ${h.isin}` : ""}</span>
                      </td>
                      ...
                      <td className="py-3 px-3 text-right">
                        <span className={`font-bold tabular-nums ${isProfitable ? "text-[var(--positive)]" : "text-[var(--negative)]"}`}>
                          {isProfitable ? "+" : ""}{h.returns.toFixed(2)}%
                        </span>
                      ...
                      <td className="py-3 px-3 text-right">
                        <span className="font-semibold text-foreground tabular-nums">{h.allocation.toFixed(1)}%</span>
                        <div className="w-14 h-1 bg-accent rounded-full overflow-hidden mt-1 ml-auto">
                          <div className="h-full bg-slate-400 dark:bg-slate-600 rounded-full" style={{ width: `${h.allocation * 4}%` }} />
                        </div>
                      </td>
              ...
              <div>
                ...
                {selectedHolding.isin && (
                  <p className="text-xs text-muted-foreground font-mono mt-0.5">
                    ISIN: {selectedHolding.isin} {selectedHolding.folioNumber ? `· Folio: ${selectedHolding.folioNumber}` : ""}
                  </p>
                )}
              </div>
            ...
            <div className="p-6 space-y-6 overflow-y-auto">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="p-3.5 rounded-xl bg-accent/40 border border-border">
                  <span className="text-[11px] text-muted-foreground block">Current Value</span>
                ...
                <div className="p-3.5 rounded-xl bg-accent/40 border border-border">
                  <span className="text-[11px] text-muted-foreground block">Invested Amount</span>
                  <span className="text-base font-bold font-finance text-foreground tabular-nums">
                    ₹{(selectedHolding.avgPrice * selectedHolding.quantity).toLocaleString("en-IN")}
                  </span>
                </div>

                <div className="p-3.5 rounded-xl bg-accent/40 border border-border">
                  <span className="text-[11px] text-muted-foreground block">Total Returns</span>
                  <span className={cn(
                    "text-base font-bold font-finance tabular-nums",
                    selectedHolding.returns >= 0 ? "text-[var(--positive)]" : "text-[var(--negative)]"
                  )}>
                    {selectedHolding.returns >= 0 ? "+" : ""}{selectedHolding.returns.toFixed(2)}%
                  </span>
                </div>

                <div className="p-3.5 rounded-xl bg-accent/40 border border-border">
                  <span className="text-[11px] text-muted-foreground block">Portfolio Weight</span>
                  <span className="text-base font-bold font-finance text-foreground tabular-nums">
              ...
              <div className="p-4 rounded-xl border border-border bg-accent/20 space-y-2.5 text-xs font-finance">
                ...
                <div className="flex justify-between">
                  <span className="text-muted-foreground font-sans">Quantity / Units:</span>
                  <span className="font-semibold text-foreground">{selectedHolding.quantity} Units</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground font-sans">Average Buy Price:</span>
                  <span className="font-semibold text-foreground">₹{selectedHolding.avgPrice.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground font-sans">Current LTP / NAV:</span>
                  <span className="font-semibold text-foreground">₹{selectedHolding.currentPrice.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground font-sans">Nominee Status:</span>
                  <span className={cn(
                    "font-semibold",
                    selectedHolding.nomineeStatus === "Verified" ? "text-[var(--positive)]" : "text-amber-500"
                  )}>
              ...
              {selectedHolding.planType === "Regular" && (
                <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/25 space-y-2 text-xs">
                  ...
                  <p className="text-muted-foreground text-[11.5px] leading-relaxed">
                    This fund is currently running as a Regular Plan with distributor commissions embedded in the NAV. Switching this holding to a <strong>Direct Plan</strong> can save you ~₹4,200 annually and add significantly to your compounding wealth.
                  </p>
                </div>
              )}
            </div>
